...
from pyShicy import CMD_HELP, HOSTED_ON, shicy_ver
...
async def give_next_page(_, cb: CallbackQuery):
    ...
    btn = chiy.HelpXd(current_page_number + 1, CMD_HELP, "xd")
...
async def get_back(_, cb: CallbackQuery):
    ...
    btn = chiy.HelpXd(page_number, CMD_HELP, "xd")
...
async def give_plugin_cmds(_, cb: CallbackQuery):
    ...
    plugs = await chiy.PluginXd(CMD_HELP, plugin_name)
...
async def update_callback(_, cb: CallbackQuery):
    ...
    if HOSTED_ON == "Heroku":
        ...
        try:
            await cb.edit_message_text(
            "<b>[HEROKU]:</b> <i>Update Deploy ShicyUbot Sedang Dalam Proses...</i>"
            )
        except:
            pass
        ups_rem.fetch(ac_br)
        ...
        try:
            await cb.edit_message_text(
            "<i>ShicyUbot Berhasil Diupdate! Userbot bisa di Gunakan Lagi.</i>"
            )
        ...
        try:
            ...
            repo.git.reset("--hard", "FETCH_HEAD")
        await chiy.install_requirements()
        try:
            await cb.edit_message_text(
            "<i>ShicyUbot Berhasil Diupdate! Userbot bisa di Gunakan Lagi.</i>",
            )
...
async def changelog_callback(client, cb: CallbackQuery):
    msg = cb.message
    changelog, tl_chnglog = await chiy.gen_chlog(
    ...
    if changelog:
        if len(changelog) > 4096:
            ...
            await client.send_document(
                ...
                reply_to_message_id=chiy.ReplyCheck(msg),
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
...
async def get_back(client: Client, cb: CallbackQuery):
    ...
    await chiy.approve_pmpermit(cb, user_ids, OLD_MSG)


@callback(pattern="tolak_(.*)", client_only=True)
async def get_back(client: Client, cb: CallbackQuery):
    user_ids = int(cb.matches[0].group(1))
    await chiy.disapprove_pmpermit(cb, user_ids)
...
async def added_to_group_msg(bot, cq):
    ...
    try:
        string_session = await chiy.generate_premium(
            bot,
            Var.LOG_CHAT,
            f"ShicyUbot {shicy_ver}",
            cq.message,
        )
        if HOSTED_ON == "Heroku":
            ...
            if not dotenv.get_key(path, vars):
                ...
                os.execvp(sys.executable, [sys.executable, "-m", "pyShicy"])
            else:
                pass
    except Exception as e:
        return await cq.message.reply("[ERROR]\n{}\nSilahkan Teruskan Pesan Ini Ke @ShicyyXCode".format(str(e)))
...
async def added_to_group_msg(bot, cq):
    ...
    try:
        ...
        return await cq.message.reply("[ERROR]\n{}\nSilahkan Teruskan Pesan Ini Ke @ShicyChat".format(str(e)))
    await cq.message.delete()
